...
def get_openai_response(input_text):
    response = requests.post("http://localhost:9000/assay/invoke", json={'input': {'topic': input_text}})
    return response.json()['output']['content']


def get_ollama_response(input_text):
    response = requests.post("http://localhost:9000/poem/invoke", json={'input': {'topic': input_text}})
